...
def pull_log_files():
    ...
    query =  "/services/data/v44.0/query?q=SELECT+Action+,+CreatedById+,+CreatedDate+,+DelegateUser+,+Display+,+Id+,+Section" + \
                "+FROM+SetupAuditTrail"   + \
                f"+WHERE+CreatedDate+>+{past_time}"
    try:
...
def main(mytimer: func.TimerRequest) -> None:
    ...
    for line in pull_log_files():
        logging.info('Started downloading {}'.format(line["attributes"]['type']))
        del line['attributes']
        sentinalout.append(line)
    body = json.dumps(sentinalout)
    post_data(customer_id, shared_key, body, log_type, len(sentinalout))
